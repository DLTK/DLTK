...
from dltk.networks.regression_classification.flexnet import flexnet_3d
...
def model_fn(features, labels, mode, params):
    ...
    vars = np.sum([np.prod(v.get_shape().as_list()) for v in
                   tf.trainable_variables()])
    print('Got {} variables'.format(vars))
    ...
    if params['unit_type'] == 'variational_se_resnext':
        loss += tf.reduce_sum(tf.get_collection('variational_se_kl'))
...
def train(args):
    ...
    nn = tf.estimator.Estimator(
        ...
        params={"learning_rate": 0.001, 'unit_type': args.unit_type},
...
if __name__ == '__main__':
    ...
    parser.add_argument('--unit_type', '-u', default='residual',
                        choices=['residual', 'resnext', 'dense',
                                 'se_residual', 'se_resnext'])

    args = parser.parse_args()

    args.model_path = os.path.join(args.model_path, args.unit_type)
