...
from dltk.core.activations import *

def crop_central_block(x, size):
    assert all([i >= s for i, s in zip(x.get_shape().as_list()[1:], size)]), \
    'Output size must not be bigger than input size. But was {} compared to {}'.format(
        x.get_shape().as_list()[1:], size)
...
def deepmedic_3D(inputs, num_classes,
                 ...
                 subsampled_residuals=((4, 6, 8),), subsampled_input_shapes=((57, 57, 57),),
                 subsample_factors=((3, 3, 3),), fc_filters=(150, 150),
                 first_fc_kernel=(3, 3, 3), fc_residuals=(2, ),
                 padding='VALID', use_prelu=True,
    ...
    [1] Konstantinos Kamnitsas et al. Efficient Multi-Scale 3D CNN with Fully Connected CRF for Accurate Brain Lesion Segmentation. Medical Image Analysis, 2016.
    [2] Konstantinos Kamnitsas et al. Multi-Scale 3D CNNs for segmentation of brain Lesions in multi-modal MRI. ISLES challenge, MICCAI 2015.
    ...
    def _build_normal_pathway(x):
        with tf.variable_scope('normal_pathway'):
            ...
            tf.logging.info('Input is {}'.format(center_crop.get_shape().as_list()))
            ...
            for i in range(len(normal_filters)):
                with tf.variable_scope('layer_{}'.format(i)):
                    ...
                    x = tf.layers.conv3d(x, normal_filters[i], normal_kernels[i], normal_strides[i], **conv_params)
                    # TODO: add pooling and dropout?!
                    if i + 1 in normal_residuals:
                        x = _residual_connection(x, layers[i - 1])
                    tf.logging.info('Output of layer {} is {}'.format(i, x.get_shape().as_list()))
    ...
    def _build_subsampled_pathways(x):
        ...
        for pathway in range(len(subsample_factors)):
            with tf.variable_scope('subsampled_pathway_{}'.format(pathway)):
                tf.logging.info('Building subsampled pathway {}'.format(pathway))
                center_crop = crop_central_block(x, subsampled_input_shapes[pathway])
                tf.logging.info('Input is {}'.format(center_crop.get_shape().as_list()))
                ...
                tf.logging.info('Downsampled input is {}'.format(x.get_shape().as_list()))
    ...
    for i in range(len(fc_filters)):
        with tf.variable_scope('fc_{}'.format(i)):
            ...
            x = tf.layers.batch_normalization(x, training=mode == tf.estimator.ModeKeys.TRAIN)
            x = prelu(x) if use_prelu else leaky_relu(x, 0.01)
            x = tf.layers.conv3d(x, fc_filters[i], first_fc_kernel if i == 0 else 1, **conv_params)
