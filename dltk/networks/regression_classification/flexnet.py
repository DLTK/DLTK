...
from dltk.core.units import *


def flexnet_3d(inputs,
               num_classes,
               num_units=2,
               unit_type='residual',
    ...
    for res_scale in range(0, len(filters) - 1):
        with tf.variable_scope('down_{}'.format(res_scale)):
            k = [s * 2 if s > 1 else 3 for s in strides[res_scale]]
            x = tf.layers.conv3d(
                x, filters[res_scale], k, strides[res_scale], **conv_params)
        tf.logging.info('Encoder at res_scale down {} tensor shape: {}'.format(
        ...
        if unit_type == 'dense':
            diff = filters[res_scale + 1] - filters[res_scale]
            growing_rate = diff // 4
            with tf.variable_scope('unit_{}'.format(res_scale + 1)):
                x = dense_unit_3d(inputs=x, out_filters=filters[res_scale + 1],
                                  growing_rate=growing_rate,
                                  num_convolutions=2 * num_units,
                                  activation=activation, mode=mode)
        else:
            for i in range(0, num_units):
                with tf.variable_scope('unit_{}_{}'.format(res_scale + 1, i)):
                    if unit_type == 'residual':
                        x = residual_unit_3d(
                            inputs=x, out_filters=filters[res_scale + 1],
                            activation=activation, mode=mode)
                    elif unit_type == 'se_residual':
                        x = se_residual_unit_3d(
                            inputs=x, out_filters=filters[res_scale + 1],
                            reduction_ratio=4,
                            activation=activation, mode=mode)
                    elif unit_type == 'resnext':
                        out_filters = filters[res_scale + 1]
                        cardinality = out_filters // 4
                        x = resnext_unit_3d(
                            inputs=x, cardinality=cardinality,
                            out_filters=out_filters,
                            activation=activation, mode=mode)
                    elif unit_type == 'se_resnext':
                        out_filters = filters[res_scale + 1]
                        cardinality = out_filters // 4
                        x = se_resnext_unit_3d(
                            inputs=x, cardinality=cardinality,
                            reduction_ratio=4, out_filters=out_filters,
                            activation=activation, mode=mode)
                    elif unit_type == 'variational_se_resnext':
                        out_filters = filters[res_scale + 1]
                        cardinality = out_filters // 4
                        x = variational_se_resnext_unit_3d(
                            inputs=x, cardinality=cardinality,
                            reduction_ratio=4, out_filters=out_filters,
                            activation=activation, mode=mode)
