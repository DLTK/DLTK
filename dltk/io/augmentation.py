...
def flip(imagelist, axis=1):
    ...
    Returns:
        np.ndarray or list or tuple: same as imagelist but randomly flipped along axis
    """
...
def extract_class_balanced_example_array(image, label, example_size=[1, 64, 64], n_examples=1, classes=2, class_weights=None):
    ...
    Returns:
        np.ndarray, np.ndarray: class-balanced patches extracted from full images with 
        the shape [batch, example_size..., image_channels]
    """
    ...
    for c_idx, c in enumerate(classes):
        ...
        for i in range(len(r_idx)):
            ...
            ex_images = np.concatenate((ex_images, ex_image), axis=0) if (len(ex_images) != 0) else ex_image
...
def extract_random_example_array(image_list, example_size=[1, 64, 64], n_examples=1):
    ...
    for i in range(n_examples):
        ...
        for j in range(len(image_list)):
            ...
            examples[j] = np.concatenate((examples[j], ex_image), axis=0) if (len(examples[j]) != 0) else ex_image
