...
def elastic_transform(image_list, alpha, sigma, random_seed=None):
    """
    ...
    Args:
        image_list (list): list of images (np.ndarray) to be deformed
        ...
        random_seed: an integer to seed numpy's random generator. Used to make sure the same co-efficients are used in the mask for each image in the image list. If set to None, a different set of co-efficients is used in the mask per image.
    Returns:
        list: a simple python list of images, where each image is of type np.ndarray
    """
