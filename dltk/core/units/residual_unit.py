...
def variational_se_resnext_unit_3d(inputs,
                                   out_filters,
                                   cardinality=32,
                                   bottleneck_channels=4,
                                   reduction_ratio=16,
    ...
    with tf.variable_scope('sub_se_unit'):
        ...
        scale_mu = tf.layers.dense(scale, out_filters)

        scale_log_sig_sq = tf.layers.dense(scale, out_filters)

        sampled_scale = (scale_mu + tf.random_normal(tf.shape(scale_mu))
                         * tf.sqrt(tf.exp(scale_log_sig_sq)))

        kl_div = -0.5 * tf.reduce_mean(
            tf.reduce_sum(1 + scale_log_sig_sq - tf.square(scale_mu)
                          - tf.exp(scale_log_sig_sq), 1))

        tf.add_to_collection('variational_se_kl', kl_div)

        scale = tf.reshape(sampled_scale, [-1, 1, 1, 1, out_filters])
