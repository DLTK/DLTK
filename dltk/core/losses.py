...
def sparse_balanced_crossentropy(logits, labels):
    ...
    Returns:
        tf.Tensor: Tensor representing the loss
    """
    ...
    class_frequencies = tf.stop_gradient(tf.bincount(labels, minlength=num_classes, dtype=tf.float32))
    weights = (1. / (class_frequencies + tf.constant(1e-8))) * (tf.cast(tf.reduce_prod(tf.shape(labels)), tf.float32)
...
def dice_loss(logits, labels, num_classes, smooth=1e-5, include_background=True, only_present=False):
    ...
    # Get a softmax probability of the logits predictions and a one hot encoding of the labels tensor
    probs = tf.nn.softmax(logits)
    ...
    if include_background:
        flat_per_sample_per_class_dice = tf.reshape(per_sample_per_class_dice, (-1, ))
        flat_label = tf.reshape(label_sum, (-1, ))
    else:
        flat_per_sample_per_class_dice = tf.reshape(per_sample_per_class_dice[:, 1:], (-1, ))
